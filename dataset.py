...
def get_dataloaders():
    ...
    def save_samples(dataset, filename, num_samples=500):
        with open(filename, 'w') as f:
            for i in range(min(num_samples, len(dataset))):
                inputs, labels = dataset[i]
                f.write(f"样本 {i}:\n")
                f.write(f"题目ID: {inputs['input_ids']}\n")
                f.write(f"时间序列: {inputs['input_rtime']}\n")
                f.write(f"题目类别: {inputs['input_cat']}\n")
                f.write(f"标签答案: {labels}\n")
                f.write("-" * 50 + "\n")

    # 在get_dataloaders()中添加
    # save_samples(train_dataset, "train_samples.txt")
    # save_samples(val_dataset, "val_samples.txt")
